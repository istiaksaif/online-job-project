...
import com.simplemobiletools.gallery.bayzid.BuildConfig
import com.simplemobiletools.gallery.bayzid.R
import com.simplemobiletools.gallery.bayzid.adapters.DirectoryAdapter
import com.simplemobiletools.gallery.bayzid.adapters.MediaAdapter
import com.simplemobiletools.gallery.bayzid.asynctasks.GetMediaAsynctask
import com.simplemobiletools.gallery.bayzid.databases.GalleryDatabase
import com.simplemobiletools.gallery.bayzid.dialogs.ChangeSortingDialog
import com.simplemobiletools.gallery.bayzid.dialogs.ChangeViewTypeDialog
import com.simplemobiletools.gallery.bayzid.dialogs.FilterMediaDialog
import com.simplemobiletools.gallery.bayzid.extensions.*
import com.simplemobiletools.gallery.bayzid.helpers.*
import com.simplemobiletools.gallery.bayzid.interfaces.DirectoryOperationsListener
import com.simplemobiletools.gallery.bayzid.interfaces.MediaOperationsListener
import com.simplemobiletools.gallery.bayzid.jobs.NewPhotoFetcher
...
class MainActivity : SimpleActivity(), DirectoryOperationsListener, MediaOperationsListener {
    ...
    private var mAllowPickingMultiple = true
...
//        checkWhatsNewDialog()
    ...
    private fun showFilterMediaDialog1() {
...
//            finish()
    ...
    private fun startAsyncTask() {
        ...
        mCurrAsyncTask = GetMediaAsynctask(applicationContext, mPath, mIsGetImageIntent, mIsGetVideoIntent, mShowAll) {
            ensureBackgroundThread {
                val oldMedia = MediaActivity.mMedia.clone() as ArrayList<ThumbnailItem>
                val newMedia = it
                try {
    ...
    private fun gotMedia(media: ArrayList<ThumbnailItem>, isFromCache: Boolean) {
        ...
        MediaActivity.mMedia = media

        runOnUiThread {
            directories_refresh_layout.isRefreshing = false
            directories_empty_placeholder.beVisibleIf(media.isEmpty() && !isFromCache)
            directories_empty_placeholder_2.beVisibleIf(media.isEmpty() && !isFromCache)

            if (directories_empty_placeholder.isVisible()) {
...
//            overridePendingTransition(0, 0);
        ...
        if (!isFromCache) {
            val mediaToInsert = (MediaActivity.mMedia).filter { it is Medium && it.deletedTS == 0L }.map { it as Medium }
            Thread {
                try {
    ...
    private fun setupAdapter1() {
        ...
        if (currAdapter == null) {
            initZoomListener()
            MediaAdapter(
                this, MediaActivity.mMedia.clone() as ArrayList<ThumbnailItem>, this, mIsGetImageIntent || mIsGetVideoIntent || mIsGetAnyIntent,
                mAllowPickingMultiple, mPath, directories_grid
            ) {
            ...
            //  (currAdapter as DirectoryAdapter).updateMedia(MainActivity.mMedia)
            handleGridSpacing()
        } else {
    ...
    private fun searchQueryChanged(text: String) {
        ensureBackgroundThread {
            try {
                val filtered = MainActivity.mMedia.filter { it is Medium && it.name.contains(text, true) } as ArrayList
    ...
    private fun getMediaAdapter() = directories_grid.adapter as? DirectoryAdapter


    private fun isDirEmpty(): Boolean {
        return if (MediaActivity.mMedia.size <= 0 && config.filterMedia > 0) {
    ...
    private fun handleGridSpacing(media: ArrayList<ThumbnailItem> = MediaActivity.mMedia) {
        val viewType = config.getFolderViewType(if (mShowAll) SHOW_ALL else mPath)
        if (viewType == VIEW_TYPE_GRID) {
            ...
            if (directories_grid.itemDecorationCount > 0) {
                currentGridDecoration = directories_grid.getItemDecorationAt(0) as GridSpacingItemDecoration
            ...
            if (currentGridDecoration.toString() != newGridDecoration.toString()) {
                if (currentGridDecoration != null) {
                    directories_grid.removeItemDecoration(currentGridDecoration)
                }
                directories_grid.addItemDecoration(newGridDecoration)
            }
        }
    }
...
//        var result = 0
//        if (this.filter_media_images.isChecked)
//            result += TYPE_IMAGES
//        if (this.filter_media_videos.isChecked)
//            result += TYPE_VIDEOS
//        if (this.filter_media_gifs.isChecked)
//            result += TYPE_GIFS
//        if (this.filter_media_raws.isChecked)
//            result += TYPE_RAWS
//        if (this.filter_media_svgs.isChecked)
//            result += TYPE_SVGS
//        if (this.filter_media_portraits.isChecked)
